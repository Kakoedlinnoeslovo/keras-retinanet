...
def _findNode(parent, name, debug_name = None, parse = None):
    ...
    if parse is not None:
        try:
            ...
            raise ValueError('invalid format for {}: {}'.format(debug_name, e))
    return result
...
class PascalVocGenerator(Generator):
    ...
    def __parse_annotation(self, element):
        ...
        if class_name not in self.classes:
            raise ValueError('class name "{}" not found in classes {}'.format(class_name, self.classes))
    ...
    def __parse_annotations(self, xml_root):
        ...
        for i, element in enumerate(root.iter('object')):
            try:
                truncated, difficult, box = self.__parse_object(element)
            except ValueError as e:
                raise ValueError('invalid object #{}: {}'.format(i, e))
